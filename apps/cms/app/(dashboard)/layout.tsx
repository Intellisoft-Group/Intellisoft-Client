...
import { allowedPath, homeFor, navFor, navLinkActive, roleWorkspaceLabel } from '@/lib/roles';

/** Calm gate — no shimmering skeleton shell (that flashed after login). */
function AuthGate() {
  return (
    <div className="auth-gate" aria-busy="true" aria-live="polite">
      <p className="page-loading">Loading workspace…</p>
    </div>
  );
}
...
function DashboardLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const workspace = roleWorkspaceLabel(user.role);
    ...
    <div className={`shell ${open ? 'nav-open' : ''}`}>
      <header className="mobile-topbar">
        <div className="mobile-topbar-brand">
          ...
          <span className="mobile-topbar-role">{workspace}</span>
        </div>
        <div className="mobile-topbar-actions">
          {!open ? (
            <button
              className="nav-toggle ui"
              type="button"
      ...
      <aside className={`sidebar ui ${open ? 'open' : ''}`}>
        ...
        {nav.map((item, i) =>
          item.group ? (
            <div className="nav-group" key={`${item.group}-${i}`}>{item.group}</div>
          ) : (
            <Link
              key={item.href}
              href={item.href!}
              className={`nav-link ${navLinkActive(pathname, search, item.href!) ? 'active' : ''}`}
            >
