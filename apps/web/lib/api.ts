...
export function getToken() {
  ...
  return localStorage.getItem('is_client_token');
}

function getRefreshToken() {
  if (typeof window === 'undefined') return null;
  return localStorage.getItem('is_client_refresh');
}
...
export function clearSession() {
  localStorage.removeItem('is_client_token');
  localStorage.removeItem('is_client_refresh');
  localStorage.removeItem('is_client_user');
}

export function getUser<T = any>(): T | null {
  if (typeof window === 'undefined') return null;
  const raw = localStorage.getItem('is_client_user');
  if (!raw) return null;
  try {
...
export async function api<T = any>(path: string, opts: ApiOpts = {}): Promise<T> {
  ...
  try {
    ...
    throw new Error('Unable to connect to Intellisoft. Please try again in a moment.');
  }
